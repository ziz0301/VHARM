	  ...
	  // Function to execute button: "Clear first!" from the main page
	   ...
	   // Calling function to give IVHARM the JSON data
		...
		function processFirstData(inputobj){
			...
			//console.log("Nod with vulnerabilities")
			...
			upperLinktemp.forEach(obj=>{
				...
				if(nodefrominfo !== undefined){
					if(nodefrominfo['group'] == 'HU' || nodefrominfo['group'] == 'TCB' || nodefrominfo['group'] == 'IS'|| nodefrominfo['group'] == 'GW'){
				...
				if(nodetoinfo !== undefined){
					if(nodetoinfo['group'] == 'HU' || nodetoinfo['group'] == 'TCB' || nodetoinfo['group'] == 'IS'|| nodetoinfo['group'] == 'GW'){
